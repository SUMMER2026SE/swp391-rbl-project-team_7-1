      ...
      <section className="w-80 border-r border-slate-200 bg-white flex flex-col shrink-0 shadow-sm z-10">
        <div className="p-6 border-b border-slate-100">
          ...
          <div className="relative">
            ...
            <input className="w-full pl-11 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-teal-600/20 focus:border-teal-600 transition-all font-medium text-slate-700" placeholder="Tìm kiếm tin nhắn..." type="text"/>
          </div>
        </div>
            ...
            <div className="flex gap-4">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex justify-between items-baseline mb-1">
                  ...
                  <span className="text-xs text-slate-400 font-medium">10:42 SA</span>
                </div>
                <p className="text-sm text-slate-700 truncate font-semibold">Đây là bản mockup cuối cùng cho...</p>
                <p className="text-xs text-teal-700 mt-1.5 truncate font-medium bg-white px-2 py-0.5 rounded border border-teal-100 w-fit">Phát triển Ứng dụng Di động</p>
              </div>
            </div>
          </div>
          {/*  Inactive Chat Items  */}
          <div className="p-5 border-b border-slate-100 bg-white hover:bg-slate-50 cursor-pointer transition-colors">
            <div className="flex gap-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-sm text-slate-500 truncate">Tôi sẽ hoàn thành backend vào thứ Sáu.</p>
                <p className="text-xs text-slate-500 mt-1.5 truncate font-medium bg-slate-100 px-2 py-0.5 rounded border border-slate-200 w-fit">Dự án tích hợp API</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="flex-1 flex flex-col bg-slate-50 relative">
          ...
          <div className="flex items-center gap-5">
            ...
            <div>
              ...
              <div className="flex items-center gap-3 text-sm text-slate-500 font-medium">
                <span>Phát triển Ứng dụng Di động</span>
                <span className="w-1 h-1 rounded-full bg-slate-300"></span>
                <span className="flex items-center gap-1.5 text-teal-600"><span className="w-2 h-2 rounded-full bg-teal-500 shadow-[0_0_8px_rgba(20,184,166,0.6)]"></span> Đang trực tuyến</span>
              </div>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-8 space-y-6 custom-scrollbar bg-slate-50 flex flex-col">
          ...
          <div className="flex justify-center my-4">
            <span className="bg-white text-slate-400 text-xs font-bold uppercase tracking-wider px-4 py-1.5 rounded-full border border-slate-200 shadow-sm">Hôm nay</span>
          </div>
          ...
          <div className="flex gap-4 max-w-2xl self-end">
            <div className="bg-[#0F766E] text-white rounded-2xl rounded-tr-sm p-5 shadow-sm border border-teal-800">
              <p className="text-base leading-relaxed">Chào Elena, bản mockup tiến triển thế nào rồi? Chúng tôi hy vọng có thể trình bày với các bên liên quan vào chiều mai.</p>
              <span className="text-xs text-teal-100 block mt-2 text-right font-medium">09:15 SA</span>
            </div>
          </div>
          ...
          <div className="flex gap-4 max-w-2xl">
            ...
            <div className="space-y-2">
              <div className="bg-white text-slate-700 rounded-2xl rounded-tl-sm p-5 shadow-sm border border-slate-200">
                <p className="text-base leading-relaxed mb-4">Chào bạn! Chúng đã sẵn sàng. Tôi đã tinh chỉnh lại bảng màu như chúng ta đã thảo luận và đảm bảo các vùng chạm được tối ưu hóa cho di động. Cho tôi biết ý kiến của bạn nhé!</p>
                {/*  File Attachment  */}
                <div className="bg-slate-50 border border-slate-200 rounded-xl p-3 flex items-center gap-4 hover:bg-slate-100 transition-colors group cursor-pointer">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-slate-500 font-medium mt-0.5">24.5 MB • Tập tin ZIP</p>
                  </div>
                ...
                <span className="text-xs text-slate-400 block mt-3 font-medium">10:42 SA</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="p-5 bg-white border-t border-slate-200 z-10 shadow-[0_-4px_20px_rgba(15,23,42,0.02)]">
          ...
          <div className="flex gap-3 mb-4 overflow-x-auto custom-scrollbar pb-2">
            ...
            <button className="whitespace-nowrap px-4 py-2 bg-slate-50 text-slate-600 hover:bg-teal-50 hover:text-teal-700 hover:border-teal-200 border border-slate-200 rounded-full text-sm font-semibold transition-all active:scale-[0.98]">
              Chúng ta có thể điều chỉnh màu sắc không?
            </button>
            <button className="whitespace-nowrap px-4 py-2 bg-slate-50 text-slate-600 hover:bg-teal-50 hover:text-teal-700 hover:border-teal-200 border border-slate-200 rounded-full text-sm font-semibold transition-all active:scale-[0.98]">
              Khi nào chúng ta có thể gọi điện?
            </button>
          </div>
          {/*  Input Box  */}
          <div className="bg-slate-50 border border-slate-200 rounded-2xl flex items-end gap-2 p-2 focus-within:ring-2 focus-within:ring-teal-600/20 focus-within:border-teal-600 transition-all shadow-sm">
            ...
            <textarea className="flex-1 bg-transparent border-none text-base text-slate-700 focus:ring-0 resize-none py-3 font-medium max-h-32 custom-scrollbar" placeholder="Nhập tin nhắn..." rows="1"></textarea>
            <button className="p-2.5 text-slate-400 hover:text-teal-600 hover:bg-slate-200 rounded-xl transition-colors shrink-0 mb-0.5">
        ...
        <div className="absolute right-8 top-24 w-80 bg-white/95 border border-slate-200 rounded-2xl shadow-lg overflow-hidden flex flex-col z-20 backdrop-blur-md">
          <div className="bg-slate-50 px-5 py-3.5 border-b border-slate-200 flex items-center gap-2">
            ...
            <span className="font-bold text-sm text-slate-800 tracking-tight">Trợ lý FJMS</span>
          </div>
          <div className="p-5 bg-white">
            <p className="text-sm text-slate-600 mb-5 leading-relaxed font-medium">
              Elena vừa chia sẻ bản mockup. Bạn có muốn nạp tiền cho giai đoạn tiếp theo <strong className="text-teal-700">1.200.000 đ</strong> qua VNPay Escrow không?
            </p>
            <div className="flex gap-3">
