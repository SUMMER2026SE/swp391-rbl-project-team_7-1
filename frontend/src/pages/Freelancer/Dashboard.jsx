...
export default function FreelancerDashboard() {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      // Fetch AI recommended projects
      try {
        const recRes = await projectService.getFreelancerRecommendedProjects();
        if (recRes?.success) {
          setRecommendedProjects(recRes.data || []);
        }
      } catch (recErr) {
        console.error('Error fetching recommended projects:', recErr);
      }
          ...
          <div className="lg:col-span-2">
            <div className="bg-white rounded-3xl border border-slate-100 shadow-[0_15px_40px_rgba(15,23,42,0.03)] overflow-hidden">
              <div className="p-6 md:p-8 border-b border-slate-100 flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                  <h2 className="text-xl font-bold text-slate-800">Bảng điều khiển công việc</h2>
                </div>
                
                <div className="flex items-center bg-slate-50 p-1 rounded-2xl border border-slate-100 shrink-0 flex-nowrap overflow-x-auto gap-1">
                  <button 
                    onClick={() => setActiveTab('contracts')}
                    className={`px-3.5 py-2 rounded-xl text-xs font-bold transition-all whitespace-nowrap ${
                      activeTab === 'contracts' ? 'bg-[#0F766E] text-white shadow-sm' : 'text-slate-600 hover:bg-slate-100 hover:text-slate-900'
                    }`}
                  >
                  ...
                  <button 
                    ...
                    className={`px-3.5 py-2 rounded-xl text-xs font-bold transition-all whitespace-nowrap ${
                      activeTab === 'proposals' ? 'bg-[#0F766E] text-white shadow-sm' : 'text-slate-600 hover:bg-slate-100 hover:text-slate-900'
                    }`}
                  >
                    Đề xuất ({submittedProposalsCount})
                  </button>
                  <button 
                    onClick={() => setActiveTab('invitations')}
                    className={`px-3.5 py-2 rounded-xl text-xs font-bold transition-all whitespace-nowrap ${
                      activeTab === 'invitations' ? 'bg-[#0F766E] text-white shadow-sm' : 'text-slate-600 hover:bg-slate-100 hover:text-slate-900'
                    }`}
                  >
                  ...
                  <button 
                    ...
                    className={`px-3.5 py-2 rounded-xl text-xs font-bold transition-all whitespace-nowrap ${
                      activeTab === 'recommendations' ? 'bg-[#0F766E] text-white shadow-sm' : 'text-slate-600 hover:bg-slate-100 hover:text-slate-900'
                    }`}
                  >
                    ✨ AI Gợi ý ({recommendedProjects.length})
                  </button>
                </div>
              </div>
              ...
              {loading ? (
                ...
                recommendedProjects.length === 0 ? (
                  <div className="text-center py-16 text-slate-400 px-6 flex flex-col items-center justify-center">
                    <span className="material-symbols-outlined text-[48px] text-slate-300 mb-3">psychology_alt</span>
                    <p className="text-sm font-semibold text-slate-500">Hiện chưa có dự án mở nào phù hợp với hồ sơ của bạn.</p>
                    <p className="text-xs text-slate-400 mt-1 max-w-sm">Hãy cập nhật thêm kỹ năng và tải lên CV mới trong Hồ sơ cá nhân để AI nhận diện tốt hơn.</p>
                    <Link to="/profile" className="mt-4 px-4 py-2 bg-[#0F766E] text-white text-xs font-bold rounded-xl hover:bg-[#0D5E58] transition-all">
                      Cập nhật Hồ sơ cá nhân
                    </Link>
                  </div>
                ) : (
                  <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-5 bg-slate-50/60">
                    {recommendedProjects.map((proj) => (
                      <div 
                        key={proj.projectId}
                        className="bg-white rounded-2xl p-5 border border-slate-200/80 shadow-[0_4px_16px_rgba(15,23,42,0.02)] hover:shadow-[0_12px_28px_rgba(15,118,110,0.09)] hover:border-[#0F766E]/30 transition-all duration-300 flex flex-col justify-between group"
                      >
                        <div>
                          <div className="flex items-center justify-between gap-2 mb-3">
                            <span className="px-2.5 py-1 rounded-lg bg-teal-50 text-[#0F766E] text-xs font-bold border border-teal-100">
                              {proj.categoryName || 'Dự án mới'}
                            </span>
                            <div className="flex items-center gap-1 bg-emerald-50 text-emerald-700 text-xs font-extrabold px-3 py-1 rounded-full border border-emerald-200/80">
                              <span className="material-symbols-outlined text-[14px] text-emerald-600">auto_awesome</span>
                              {proj.matchScore}% Tương thích
                            </div>
                          </div>

                          <Link to={`/project-details/${proj.projectId}`} className="block group-hover:text-[#0F766E] transition-colors">
                            <h3 className="font-bold text-slate-800 text-base line-clamp-1 mb-1.5">{proj.title}</h3>
                          </Link>
                          ...
                          {proj.matchReasons && proj.matchReasons.length > 0 && (
                            <div className="p-3.5 rounded-xl bg-slate-50 border border-slate-100 mb-4 space-y-2">
                              {proj.matchReasons.map((reason, idx) => (
                                <div key={idx} className="flex items-start gap-2 text-xs font-medium text-slate-700">
                                  <span className="w-1.5 h-1.5 rounded-full bg-[#0F766E] mt-1.5 shrink-0"></span>
                                  <span>{reason}</span>
                                </div>
                              ))}
                            </div>
                        ...
                        <div>
                          <div className="flex items-center justify-between text-xs pt-3 border-t border-slate-100 mb-3.5">
                            <span className="text-slate-500 font-medium">Ngân sách dự án:</span>
                            <span className="font-extrabold text-slate-900 text-sm">
                              {proj.budgetMax ? `${Number(proj.budgetMax).toLocaleString('vi-VN')} đ` : 'Thỏa thuận'}
                            </span>
                          </div>
