...
function AIReviewCard({ aiEvaluationString }) {
  ...
  return (
    <div className="mt-4 p-5 rounded-2xl border border-teal-100 bg-gradient-to-r from-teal-50/40 via-white to-white shadow-xs">
      <div className="flex items-center justify-between mb-3 border-b border-teal-100/60 pb-2.5">
        <div className="flex items-center gap-2">
          <span className="material-symbols-outlined text-teal-600 text-[20px] animate-pulse">psychology</span>
          <span className="text-xs font-bold text-teal-950 uppercase tracking-wide">Trợ lý AI Đánh giá Ứng viên</span>
        </div>
      </div>

      {quickSummary && (
        <p className="text-xs text-slate-600 italic leading-relaxed mb-3.5">
          "{quickSummary}"
        </p>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2 border-t border-slate-100/50">
        <div>
          <span className="text-[10px] font-bold text-emerald-700 uppercase tracking-wider block mb-2">✓ Ưu điểm</span>
        ...
        <div>
          <span className="text-[10px] font-bold text-rose-700 uppercase tracking-wider block mb-2">⚠ Nhược điểm</span>
          <div className="space-y-1.5">
            {(gaps || []).map((g, idx) => (
...
export default function ManageProposals() {
  ...
  const parseProposalCoverLetter = (text) => {
    try {
      if (!text || typeof text !== 'string') return { coverLetter: '', attachmentUrl: null };
      
      // Clean portfolio text block if present
      let cleanedText = text;
      const portMarker = '📁 [Dự án Portfolio đính kèm]:';
      const portIndex = cleanedText.indexOf(portMarker);
      if (portIndex !== -1) {
        cleanedText = cleanedText.substring(0, portIndex).trim();
      }

      const marker1 = '[Tệp đính kèm bổ sung]:';
      const marker2 = '[Tệp đính kèm]:';
      let index = cleanedText.indexOf(marker1);
      let markerLength = marker1.length;
      if (index === -1) {
        index = cleanedText.indexOf(marker2);
        markerLength = marker2.length;
      }
      if (index !== -1) {
        let coverLetter = cleanedText.substring(0, index);
        coverLetter = coverLetter.replace(/-{10,}\s*$/, '').trim();
        const attachmentUrl = cleanedText.substring(index + markerLength).trim();
        return { coverLetter, attachmentUrl };
      }
      return { coverLetter: cleanedText, attachmentUrl: null };
    } catch (err) {
      console.error('Error parsing cover letter:', err);
      return { coverLetter: String(text || ''), attachmentUrl: null };
    }
  };
  ...
  return (
    <main className="flex-1 min-h-screen pb-20 bg-[#F8FAFC]">
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12 pt-10">
        ...
        <div className="flex gap-4 border-b border-slate-200 mb-6">
          <button
            ...
            className={`pb-3 text-sm font-bold transition-all border-b-2 px-1 cursor-pointer bg-transparent border-none ${activeTab === 'proposals' ? 'border-[#0F766E] text-[#0F766E]' : 'border-transparent text-slate-400 hover:text-slate-600'
              }`}
          >
          ...
          <button
            ...
            className={`pb-3 text-sm font-bold transition-all border-b-2 px-1 cursor-pointer bg-transparent border-none flex items-center gap-1 ${activeTab === 'recommendations' ? 'border-[#0F766E] text-[#0F766E]' : 'border-transparent text-slate-400 hover:text-slate-600'
              }`}
          >
                ...
                sortedProposals.map((proposal) => {
                  ...
                  return (
                    <article
                      ...
                      className={`bg-white border rounded-3xl p-6 md:p-8 shadow-[0_10px_30px_rgba(15,23,42,0.015)] hover:shadow-[0_15px_35px_rgba(15,23,42,0.04)] hover:-translate-y-0.5 transition-all duration-300 relative overflow-hidden group flex flex-col md:flex-row gap-6 md:gap-8 ${proposal.status === 'REJECTED' ? 'opacity-65 border-slate-200 bg-slate-50/30' : 'border-slate-100'
                        }`}
                    >
                      ...
                      <div className="flex-shrink-0 w-full md:w-60 flex flex-col gap-3">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            ...
                            <p className="text-slate-400 text-xs font-semibold mt-0.5">{proposal.headline || 'Freelancer chuyên nghiệp'}</p>
                      ...
                      <div className="flex-grow flex flex-col gap-4">
                        ...
                        {/* Portfolios Section */}
                        {proposal.portfolios && proposal.portfolios.length > 0 && (
                          <div className="mt-4 border-t border-slate-100 pt-4">
                            <h4 className="text-xs font-extrabold text-slate-700 uppercase tracking-wider mb-3 flex items-center gap-1.5">
                              ...
                              Dự án Portfolio ({proposal.portfolios.length})
                            </h4>
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                              {proposal.portfolios.map(port => (
                                <div 
                                  key={port.portfolio_id} 
                                  onClick={() => setSelectedPortfolio(port)}
                                  className="border border-slate-100 rounded-2xl p-3 bg-white flex flex-col gap-2.5 hover:border-slate-350 hover:shadow-md transition-all shadow-[0_2px_8px_rgba(15,23,42,0.01)] cursor-pointer group"
                                >
                                  {port.image_url ? (
                                    <img
                                      src={port.image_url}
                                      alt={port.title}
                                      className="w-full h-28 object-cover rounded-xl border border-slate-50"
                                    />
                                  ) : (
                                    <div className="w-full h-28 bg-slate-50 rounded-xl flex items-center justify-center text-slate-300 border border-slate-50/60">
                                      <span className="material-symbols-outlined text-[32px]">image</span>
                                    </div>
                                  )}
                                  <div>
                                    <h5 className="text-xs font-bold text-slate-800 line-clamp-1 group-hover:text-[#0F766E] transition-colors">{port.title}</h5>
                                    <p className="text-[11px] text-slate-500 line-clamp-2 mt-0.5 leading-relaxed">{port.description}</p>
                                  </div>
                                  {port.project_url && (
                                    <div className="mt-4 flex justify-end">
                                      <a
                                        href={port.project_url}
                                        target="_blank"
                                        rel="noopener noreferrer"
                                        onClick={(e) => e.stopPropagation()}
                                        className="text-[11px] font-black text-[#0F766E] hover:underline flex items-center gap-0.5"
                                      >
                                        Xem dự án
      ...
      {selectedPortfolio && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-black/55 backdrop-blur-sm animate-fade-in">
